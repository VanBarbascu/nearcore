...
class EpochConfigOverrides:
    ...
    def override(self, epoch_config_dir):
        ...
        for protocol_version in self.existing_protocol_versions:
            overrides = self.common_overrides + self.epoch_config_overrides.get(protocol_version, "")
            if overrides == "":
                continue
            epoch_config_path = os.path.join(epoch_config_dir, f'{protocol_version}.json')
            logging.info(f'Applying overrides to {epoch_config_path}: {overrides}')
    ...
    def _create_missing_epoch_config_files(self, epoch_config_dir):
        ...
        existing_protocol_versions = set([int(re.match(r'(\d+)\.json', f).group(1)) for f in existing_epoch_config_files])
        ...
        for protocol_version in sorted(self.new_protocol_versions):
            if protocol_version in existing_protocol_versions:
                ...
                continue
            source_file = os.path.join(epoch_config_dir, f'{previous_protocol_version_file}.json')
            new_epoch_config_path = os.path.join(epoch_config_dir, f'{protocol_version}.json')
...
class NeardRunner:
    ...
    def neard_init(self, rpc_port, protocol_port, validator_id, genesis_protocol_version, epoch_config_overrides):
        ...
        if epoch_config_overrides is not None:
            EpochConfigOverrides(epoch_config_overrides).override(self.tmp_near_home_path('epoch_configs'))
    ...
    def do_new_test(self,
        ...
        if not (genesis_protocol_version is None or isinstance(genesis_protocol_version, int)):
            raise jsonrpc.exceptions.JSONRPCDispatchException(
                code=-32600, message='genesis_protocol_version argument not an int')
        if not (epoch_config_overrides is None or isinstance(epoch_config_overrides, dict)):
            raise jsonrpc.exceptions.JSONRPCDispatchException(
                code=-32600, message='epoch_config_overrides argument not a dict')
        ...
        with self.lock:
            ...
            try:
                ...
                pass
            try:
                self.neard_init(rpc_port, protocol_port, validator_id, genesis_protocol_version, epoch_config_overrides)
